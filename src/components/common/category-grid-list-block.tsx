          ...
          {error ? (
            ...
            <>
              <Carousel
                ...
                prevButtonClassName="-start-2 md:-start-2.5"
                nextButtonClassName="-end-2 md:-end-2.5"
              >
                {isLoading && !data
                  ...
                  : data?.categories?.data?.map((category) => (
                      ...
                      >
                        <CategoryListCard
                          ...
                          className="bg-skin-inverted shadow-category rounded-md"
                        />
                      </SwiperSlide>
                    ))}
              </Carousel>
            ...
            <div className="xl:flex justify-center flex-wrap -mx-1">
              {isLoading && !data
                ? Array.from({ length: 18 }).map((_, idx) => {
                    return (
                      <div
                        key={`category--key-${idx}`}
                        className="w-[25%] 2xl:w-[20%] 3xl:w-[16.666%] flex-shrink-0 p-2"
                      >
                        <CategoryListCardLoader
                          uniqueKey={`category-card-${idx}`}
                        />
